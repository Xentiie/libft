...
class HuffmanTreeNode:
    def __init__(self, symbol=-1, left=None, right=None, code=[]):
        self.symbol = symbol
        self.code = code
        self.left = left
        self.right = right
...
def huffman_add_node(root, code, symbol, code_intact):
    global node_added_n
    node_added_n += 1
    if len(code) == 0:
        if root is not None:
            return None
        return HuffmanTreeNode(symbol=symbol, code=code_intact)
    else:
        bit = code[0]
        if root is None:
            root = HuffmanTreeNode(code=code_intact)
        if bit == 0:
            root.left = huffman_add_node(root.left,code[1:], symbol, code_intact)
        else:
            root.right = huffman_add_node(root.right, code[1:], symbol, code_intact)
        return root
...
def decode_huffman(stream, output_buffer, ll_tree, dist_tree):
    ...
    while 1:
        ...
        print("bits:", stream.total_bits_read, end=" | ")
        if decode_blocks:
