...
void ft_init_bitmap(t_bitmap *bitmap, t_image *img, t_iv2 char_size, U32 line_chars_count, t_iv2 sep_size)
{
	bitmap->img = img;
...
void ft_draw_bitmap_text(t_image *out, t_iv4 rect, t_bitmap *bitmap, string str, F32 scale, t_iv2 kerning, t_color col)
{
	S32 char_size;
	S32 line_size;
	S32 chars_per_line;
	...
	ft_stretch_image(out, bitmap_debug_draw_rect, bitmap->img, ivec4(0, 0, bitmap->img->size.x, bitmap->img->size.y),
					 ft_color(255));
#endif
...
#if BITMAP_TEXT_DEBUG
		{
			t_color chars_rect_col = ft_color(0);
			t_color chars_arrows_col = ft_color(127, 0, 127, 255);
			ft_draw_rect(out, rect, chars_rect_col);
			ft_draw_rect(out, out_rect, chars_rect_col);
			ft_draw_rect(out, bitmap_debug_draw_rect, chars_arrows_col);
			ft_draw_line(out, ivec2((out_rect.x + out_rect.z) / 2, (out_rect.y + out_rect.w) / 2),
	...
	{
		ft_draw_rect(out, rect, COL_BLACK);
	}
#endif
}
